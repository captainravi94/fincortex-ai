...
def sanitize_markdown_for_reportlab(raw_text: str) -> str:
    """
    Safely converts markdown formatting into well-formed ReportLab XML tags,
    escaping reserved XML entities (&, <, >) while preventing overlapping tags
    from bullet asterisks and formula multiplication symbols.
    """
    if not raw_text:
        return ""

    # 1. Normalize LaTeX arrows and unicode markers
    text = raw_text.replace(r"$\rightarrow$", "→").replace(r"\rightarrow", "→")
    text = text.replace("👉", "→").replace("⚠️", "[!]")

    # 2. Replace bullet asterisks/dashes with a unicode bullet (•)
    # Prevents leading '*' from pairing with formula '*' to create false <i> tags
    text = re.sub(r"^\s*[\*\-]\s+", "• ", text)

    # 3. Process inline code `formula` BEFORE XML escaping:
    # Replace multiplication '*' with '×' inside code blocks so it never triggers italics
    ...
    # 4. Escape raw XML characters in surrounding text
    text = text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")

    # Restore the valid font tags created in step 3
    text = text.replace("&lt;font face='Courier'&gt;", "<font face='Courier'>").replace("&lt;/font&gt;", "</font>")

    # 5. Convert bold **text** to <b>text</b>
    text = re.sub(r"\*\*(.+?)\*\*", r"<b>\1</b>", text)

    # 6. Safely convert italics only when explicitly bound by word characters
    text = re.sub(r"\b\*([^\*\n]+?)\*\b", r"<i>\1</i>", text)

    # 7. Convert any remaining stray asterisks to standard bullet/dot
    text = text.replace("*", "•")

    # 8. Strip header hashes (#)
...
class InstitutionalPDFExporter:
    """
    Generates an institutional, audit-grade executive briefing dossier in PDF format.
    """
    ...
    def build_pdf_dossier(
        ...
        # Header Block
        story.append(Paragraph("FINCORTEX AI | EXECUTIVE FINANCIAL DOSSIER", subtitle_style))
        ...
        # Line-by-line parsing to avoid large unnested blocks
